...
| 2 | What went wrong? | `ENRICH WITH logs` — cross-signal correlation |
| 3 | Fleet CPU over time | `TIMESERIES 2 min FACET robot_id` — stacked time-series |
| 4 | Message flow: /scan | `MESSAGE FLOW FROM TOPIC` — causal span chain |
| 5 | Slowest spans | `SHOW SPAN SUMMARY` — latency comparison bar chart |
