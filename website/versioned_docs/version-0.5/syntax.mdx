...
See [`FROM <source>`](./command-reference#from-source) in the Command Reference for the full list of data sources and topic aliases.
...
| Temperature | `degC` `degF` `K` (also `°C` `°F`) | `K` |
| Memory | `B` `KB` `MB` `GB` | `B` |
...
Aggregate functions appear in `SELECT` expressions. ROSQL extends standard SQL aggregates with robot-specific functions.

### Standard aggregates

`AVG`, `COUNT`, `MAX`, `MIN`, `SUM`, `PERCENTILE` — standard SQL semantics.

### `TOPIC_RATE([topic_name])`

Average publish rate for a topic (or all topics) in Hz, from the `otel_metrics` table.
...
Rolling average over a sliding window of `window` rows. Compiles to `AVG(...) OVER (ROWS BETWEEN N PRECEDING AND CURRENT ROW)`.
