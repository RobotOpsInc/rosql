...
```
  ...
  <TabItem value="duckdb" label="DuckDB / Parquet">

Provide as a Parquet file registered under the name `robot_joint_map`. A sample fixture is included at [`examples/parquet/fixtures/robot_joint_map/robot_joint_map.parquet`](https://github.com/RobotOpsInc/rosql/tree/main/examples/parquet/fixtures/robot_joint_map).
...
| `FROM node_graph` | `node_graph_edges` | ROS2 node-graph publisher/subscriber edges |
| `FROM joints` | `joint_states` | ROS2 `/joint_states` per-joint position/velocity/effort |
...
ROSQL exposes every column as a bare field — query them directly without map access:
...
Stores ROS2 node-graph pub/sub edges — one row per node-to-node connection over a topic. Powers connectivity and QoS-compatibility queries such as finding edges where a `best_effort` publisher feeds a `reliable` subscriber (`compatible = false`).

```sql
CREATE TABLE node_graph_edges (
    timestamp      TIMESTAMPTZ NOT NULL,
    org_id         TEXT NOT NULL DEFAULT '',
    robot_id       TEXT NOT NULL,
    source_node    TEXT NOT NULL,          -- publishing node, e.g. '/lidar_driver'
    target_node    TEXT NOT NULL,          -- subscribing node, e.g. '/costmap_2d'
...
Stores ROS2 `/joint_states` samples — one row per joint per sample. Powers per-joint position/velocity/effort queries such as finding joints under high load (`effort > 10`).
