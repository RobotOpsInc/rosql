...
:::caution Not implemented in v0.4
`ANOMALY()` is not yet compiled. Use `MOVING_AVG` or `DERIVATIVE` to surface outliers.
:::
...
:::caution Not implemented in v0.4
`PATH DEVIATION` is not yet compiled. Query `/odom` directly: `FROM odom WHERE robot_id = '...' SINCE 1 hour ago`.
:::
