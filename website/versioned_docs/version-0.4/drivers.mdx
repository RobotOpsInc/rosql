...
- `topic_messages` — enables `FROM topics`, topic aliases (`FROM odom`, etc.); `PATH DEVIATION` is reserved but not yet implemented
- `mcap_metadata` — enables `FROM recordings`; `SHOW RECORDING` is reserved but not yet implemented
