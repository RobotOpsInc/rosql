...
rm -rf "$OUT"
mkdir -p "$OUT/traces" "$OUT/logs" "$OUT/metrics" "$OUT/topic_messages" \
         "$OUT/tf_states" "$OUT/mcap_metadata" "$OUT/robot_joint_map"

# Build a single DuckDB SQL script that loads all fixtures and exports to Parquet
TEMP_SQL=$(mktemp)
trap 'rm -f "$TEMP_SQL"' EXIT

for f in "$SQL_FIXTURES"/[0-9][0-9]_*.sql; do
  cat "$f" >> "$TEMP_SQL"
  echo ";" >> "$TEMP_SQL"
done
...
COPY tf_states       TO '$OUT/tf_states/tf_states.parquet'               (FORMAT PARQUET);
