...
- **DURING clause** — standalone `DURING(FROM source WHERE ...)` is implemented. The combined form `FROM traces WHERE ... DURING(...)` is **not yet wired** in the standard-query parser (silently dropped). The combo test is marked `#[ignore]` until this is fixed.
